package com.hurley.library.ui.weight.seekbar;
...
public class SeekArc extends View {
    ...
    private ArgbEvaluator mArcArdbEvaluator;
    private ArgbEvaluator mProgressArdbEvaluator;
...
//		Log.d(TAG, "Initialising SeekArc");
        ...
        mProgressEndColor = res.getColor(R.color.default_blue_light);
        ...
        if (attrs != null) {
            ...
            mArcEndColor = a.getColor(R.styleable.SeekArc_arcEndColor, mArcColor);
            mProgressColor = a.getColor(R.styleable.SeekArc_progressColor,
                    mProgressColor);
            mProgressEndColor = a.getColor(R.styleable.SeekArc_progressEndColor, mProgressColor);
            a.recycle();
        }
...
//					updateOnTouch(event);
    ...
    private void updateOnTouch(MotionEvent event) {
        ...
        if(MotionEvent.ACTION_MOVE == event.getAction()){
            if( mProgress <= mMax && mProgress > mMax * 3 / 4 ){
                if(event.getX() > this.getWidth() / 2)
                    progress = mMax;
            }
            else if(mProgress >= 0 && mProgress < mMax / 4) {
                if(event.getX() < this.getWidth() / 2)
                    progress = 0;
            }
        }
    ...
    private void setGradient() {
        float[] pos = {90/360.0f, 1};
        float cx = mArcRect.centerX();
        float cy = mArcRect.centerY();
        Matrix matrix = new Matrix();
        matrix.setRotate(mAngleOffset, cx, cy);
        if (mArcColor != mArcEndColor) {
            int[] arcColors = {mArcColor, mArcEndColor};
            mArcArdbEvaluator = new ArgbEvaluator();
            SweepGradient arcG = new SweepGradient(cx, cy, arcColors, pos);
            arcG.setLocalMatrix(matrix);
            mArcPaint.setShader(arcG);
        }
        if (mProgressColor != mProgressEndColor) {
            int[] proColors = {mProgressColor, mProgressEndColor};
            mProgressArdbEvaluator = new ArgbEvaluator();
            SweepGradient proG = new SweepGradient(cx, cy, proColors, pos);
            proG.setLocalMatrix(matrix);
            mProgressPaint.setShader(proG);
        }
    }
