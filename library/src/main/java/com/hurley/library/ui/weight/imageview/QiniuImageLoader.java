...
import com.hurley.library.log.NHLog;
...
public class QiniuImageLoader<T extends QiniuImageLoader> {
     ...
     * 这种模式下，宽高限定会被强制指定为GL10.GL_MAX_TEXTURE_SIZE
     */
    ...
    public static QiniuImageLoader Builder(final Context context, final String oriUrl) {
        return new QiniuImageLoader(context, oriUrl);
    }
    ...
    public String createQiniuUrl() {
        ...
        // %3E = URLEncoder.encoder(">", "utf-8")最大宽高
        final String maxHeightUtf8 = "%3E";

        String resizeParams = "";
        if (width > 0 || height > 0) {
            if (!TextUtils.isEmpty(opParams)) {
                if (width <= 0) {
                    // h > 0
            ...
            } else {//不需要高级操作，则调用/imageView2/ 可以生成较短的url
                if (width <= 0) {
                    // h > 0
                    resizeParams = ((this.mode == MODE_FORCE_ORIGIN) || (this.mode == MODE_FIT_XY)) ?
                            // fit xy /2/w/<Width>/h/<Height>
                            String.format("/2/h/%d", height) :
                            // center crop /1/w/<Width>/h/<Height>
                            String.format("/1/w/%d/h/%d", height, height);

                } else if (height <= 0) {
                    // w > 0
                    resizeParams = ((this.mode == MODE_FORCE_ORIGIN) || (this.mode == MODE_FIT_XY)) ?
                            // fit xy /2/w/<Width>/h/<Height>
                            String.format("/2/h/%d", width)  :
                            // center crop /1/w/<Width>/h/<Height>
                            String.format("/1/w/%d/h/%d", width, width);
                } else {
                    // h > 0 && w > 0
                    resizeParams = ((this.mode == MODE_FORCE_ORIGIN) || (this.mode == MODE_FIT_XY)) ?
                            // fit xy
                            String.format("/2/h%d/w/%d", width, height) :
                            // center crop
                            String.format("/1/w/%d/h/%d", width, height);
                }
            }
        }
        ...
        // interlace
        String interlaceParams = "";

        switch (this.format) {
            case jpg:
                interlaceParams = String.format("/interlace/%d", this.interlace);
                break;
            case webp:
            case gif:
            case png:
        ...
        if (!TextUtils.isEmpty(resizeParams) || !TextUtils.isEmpty(formatParams)) {
            if (oriUrl.contains("?ImageView") || oriUrl.contains("?imageMogr2")) {
                ...
                if (!TextUtils.isEmpty(opParams)) {
                    u = String.format("%s?imageMogr2/auto-orient%s%s%s%s", oriUrl, resizeParams, opParams, formatParams, interlaceParams);
                } else {
                    u = String.format("%s?imageView2%s%s%s", oriUrl, resizeParams, formatParams, interlaceParams);
                }
            }
